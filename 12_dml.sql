...
-- insert, update, delete, select(DQL) DQL : Data Query Language
...
-- 2. 제약조건
-- 제약조건에 따라 위배되는 값은 넣을 수 없다. (제약조건 종류 이후 확인) 
...
-- update 조건에서 subquery 사용하기
-- 아래의 서브쿼리는 오류가 발생된다.
-- 아래의 경우 오라클 및 포스트그래스 sql에서는 오류가 발생하지 않으나
...
-- delete
-- delete의 경우 제약조건을 따지지 않으나 유일하게 따지는 것은 fk제약 조건이다.
-- 우리의 실습 데이터에서 예시를 들면
...
-- 테이블의 값을 전체 삭제하는 경우
DELETE FROM tbl_menu; -- 실행하지 않는 경우가 많다.
